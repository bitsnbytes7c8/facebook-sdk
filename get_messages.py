...
query= "SELECT thread_id,updated_time FROM thread WHERE viewer_id=me() AND folder_id=0 LIMIT 51"
...
for name in sender_names.keys():
  try:
    print str(sender_names[name])+"\t"+name;
  except(TypeError):
    sys.stderr.write("Error: User not found - probably a group message\n");
